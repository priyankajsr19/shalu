<div class=" product_card">
    ...
    {else}
        {if in_array("buy1get1", $productitem.tags)}
    ...
    <a href="{$productitem.product_link}">
        <span class="product_image_large" href="{$productitem.product_link}" title="{$productitem.name|escape:html:'UTF-8'}">
            {if isset($lazy) && $lazy == 1}
                <img src="{$img_ps_dir}blank.jpg" data-href="{$productitem.image_link_list}" height="342" width="250" alt="{$productitem.name|escape:html:'UTF-8'}"  class="lazy"/>
                <noscript>
                <img src="{$productitem.image_link_list}" height="342" width="250" alt="{$productitem.name|escape:html:'UTF-8'}" />
                </noscript>
            {else}
                <img src="{$productitem.image_link_list}" height="342" width="250" alt="{$productitem.name|escape:html:'UTF-8'}" />
            {/if}
        ...
        <span class="product_info">
            {if $price_tax_country == 110}
                ...
                {if ($productitem.discount > 0)}
                    ...
                    <span style="color:#DA0F00;">({$productitem.discount}% Off)</span>
                {/if}
            {else}
    ...
    <a id="ajax_id_product_{$productitem.id_product}" class="quick_view_link fancybox.ajax quickview" rel="nofollow" href="{$base_dir}quickview.php?id={$productitem.id_product}">QUICK VIEW</a>
</div>
