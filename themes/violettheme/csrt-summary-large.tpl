...
{* MOD: removing the steps header from shopping cart
...
<p style="text-align:center">{l s='Your shopping bag contains'} <span id="summary_products_quantity">{$productNumber} {if $productNumber == 1}{l s='product'}{else}{l s='products'}{/if}</span></p>
<div id="co_content" style="{if $products|count > 4}width:735px;{else}width:715px;{/if}">
	<div id="co_left_column" style="{if $products|count > 4}width:730px;{else}width:710px;{/if}padding-bottom:0;">
		<div id="order-detail-content" class="table_block" style="{if $products|count > 4}width:730px;height:400px;overflow:auto{else}width:710px{/if};">
		...
		*}
					...
					<tr class="cart_free_shipping" {if $free_ship <= 0 || $isVirtualCart} style="display: none;" {/if}>
							<td colspan="6" style="white-space: normal;text-align:center;color:#DA0F00;"><span id="free_shipping_balance" class="price" style="color:#DA0F00;">{displayPrice price=$free_ship}</span> away from free shipping.</td>
							{*<td id="free_shipping" class="price" style="color:#DA0F00;">{displayPrice price=$free_ship}</td>*}
						</tr>
						...
						{foreach from=$customizedDatas.$productId.$productAttributeId key='id_customization' item='customization'}
							<tr id="product_{$product.id_product}_{$product.id_product_attribute}_{$id_customization}" class="alternate_item cart_item">
								...
								<td class="cart_quantity">
									...
									<div id="cart_quantity_button" style="position:relative;width:55px" class="qty_spinner">
			...
			{if sizeof($discounts)}
				...
				{foreach from=$discounts item=discount name=discountLoop}
					<tr class="cart_discount {if $smarty.foreach.discountLoop.last}last_item{elseif $smarty.foreach.discountLoop.first}first_item{else}item{/if}" id="cart_discount_{$discount.id_discount}">
						...
						<td class="cart_discount_delete" style="text-align:left"><a href="{if $opc}{$link->getPageLink('order-opc.php', true)}{else}{$link->getPageLink('order.php', true)}{/if}?deleteDiscount={$discount.id_discount}" title="{l s='Delete'}"><img src="{$img_dir}icon/delete.gif" alt="{l s='Delete'}" class="icon" width="11" height="13" /></a></td>
		...
		*}
		...
		<div id="cart_navigation" class="cart_navigation">
			<a rel="nofollow" href="{$link->getPageLink('order.php', true)}?step=0{if $back}&amp;back={$back}{/if}" 
				class="revieworder"></a>
			<a rel="nofollow" id="continue_shopping" href="#" 
		...
		<div class="co_rht_box">
	   		...
	   		<table><tbody>
	   			...
	   			<tr>
	   				...
	   				<td class="row_val">:<span>{$productNumber}</span></td>
	   			</tr>
	   			<tr>
	   				<td class="row_title">Sub Total</td>
	   				<td class="row_val">:<span>{displayPrice price=$total_products_wt}</span></td>
	   			</tr>
	   			<tr>
	   				<td class="row_title">Shipping</td>
	   				{if $shippingCost > 0}
	   					<td class="row_val">:<span>{displayPrice price=$shippingCost}</span></td>
	   				{else}
	   					<td class="row_val"><span style="color:green;">: FREE</span></td>
	   				{/if}
	   			</tr>
	   			<tr><td height="5px" colspan="2"></td></tr>
	   			<tr>
	   				<td class="row_title"><span style="font-size: 14px; color:#5f5f5f;">Order Total</span></td>
	   				<td class="row_val">:<span style="font-size: 14px;">{displayPrice price=$total_price}</span></td>
	   			</tr>
	   			{/if}
			</tbody></table>
	   		</div>
	   	...
	   	<div class="co_rht_box">
	   		<div id="free_shipping_info" class="rht_box_info">
	   			
	   			<span style="font-size: 13px; padding:5px 5px 5px 0px;">{displayPrice price=$free_ship}</span>
