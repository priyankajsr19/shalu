	...
	$.ajax({
       ...
       success: function(jsonData)
       {
    		...
    		else
    		{
    			...
    			{
	    			$('#co_content').children().each(function() {
	    			...
	    			$('#cart_navigation').fadeOut().remove('slow', function () {
		                $(this).remove();
			          });
					$('#summary_products_label').remove();
					$('#emptyCartWarning').fadeIn('slow');
					$('#order').delay(2000).queue(function(){
...
function updateCartSummary(json)
{
	...
	if (json.free_ship > 0 && !json.is_virtual_cart)
	{
		...
		$('#free_shipping_balance').html(formatCurrency(json.free_ship, currencyFormat, currencySign, currencyBlank));
	}
	else
