...
// <![CDATA[
var idSelectedCountry = {if isset($smarty.post.id_country)}{$smarty.post.id_country|intval}{else}{if isset($address->id_country)}{$address->id_country|intval}{else}false{/if}{/if};
var idSelectedState = {if isset($smarty.post.id_state)}{$smarty.post.id_state|intval}{else}{if isset($address->id_state)}{$address->id_state|intval}{else}false{/if}{/if};
countries = new Array();
	...
	<div id="co_left_column">
			...
			<div id="new_address_form" style="display:{if (isset($addresses) AND $addresses|@count > 0) AND !((isset($errors) && $errors) || (isset($id_address) && $id_address))}none{else}block{/if};width:650px;margin:auto;">
				...
				<form id="form_new_address" action="{$base_dir_ssl}order.php" method="post" class="std">
					<fieldset>
						...
						<p class="required text">
							<label for="postcode">{l s='Post/Zip code'}<sup>*</sup></label>
						...
						<p class="required text">
							<label for="phone_mobile">{l s='Phone'}<sup>*</sup></label>
						...
						<p class="required text" id="adress_alias" style="display:none;">
							<label for="alias" >{l s='Address book title'}<sup>*</sup></label>
		...
		{if (isset($addresses) AND $addresses|@count > 0) AND !(isset($errors) && $errors)}
			<div id="add_button_wrapper" style="width:100%">
				<span id="prev_shipaddress_label" style="width:570px;display:block;float:left;clear:both;padding:5px 10px;font-size:15px;{if isset($id_address) && $id_address}display:none{/if}">Please choose a shipping address or enter a new address.</span> 
				{literal}
					<script>
					// <![CDATA[
		...
		<form action="{$link->getPageLink('order.php', true)}" method="post" id="saved_addresses_form">
				{if isset($addresses) AND $addresses|@count > 0}
					<div id="address_wrapper">
						...
						<div id="address_link_{$address.id_address|intval}" class="selectable address_card" style="{if isset($id_address) && $id_address && $id_address == $address.id_address}display:none{/if}">
							<div class="address_title underline" style="padding:3px 15px;display:block;">
							<span style="font-size:12px;"><a class="address_update" href="{$base_dir_ssl}order.php?step=1&id_address={$address.id_address|intval}" title="{l s='Update'}" rel="address_link_{$address.id_address|intval}">[{l s='Update'}]</a></span>
							</div>
