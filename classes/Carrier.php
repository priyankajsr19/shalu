...
class CarrierCore extends ObjectModel
{
	...
	**/
	...
	public static function getPreferredCarriers($id_country, &$carriers = false)
	{
		$carriers = array('service' => array());
		$op = 0;
		$resultCountries = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('SELECT 
			priority,
			aramex, 
			ups, 
			fedex,
			bluedart
			FROM `ps_carrier_countries`
			WHERE id_country = '.$id_country);
		
		if(sizeof($resultCountries))
		{
			$countryRecord = $resultCountries[0];
			$priority = explode(',', $countryRecord['priority']);
			
			foreach($priority as $id_carrier)
			{
				if($id_carrier == UPS && intval($countryRecord['ups']))
				{
					$carriers['service'][] = UPS;
					if(!$op)
					$op = UPS;
				}
				else if($id_carrier == ARAMEX && intval($countryRecord['aramex']))
				{
					$carriers['service'][] = ARAMEX;
					if(!$op)
					$op = ARAMEX;
				}
				else if($id_carrier == FEDEX && intval($countryRecord['fedex']))
				{
					$carriers['service'][] = FEDEX;
					if(!$op)
					$op = FEDEX;
				}
				else if($id_carrier == BLUEDART && intval($countryRecord['bluedart']))
				{
					$carriers['service'][] = BLUEDART;
					if(!$op)
						$op = BLUEDART;
				}
			}
