...
class CategoryCore extends ObjectModel
{
	...
	/**
	  ...
	  */
	public function getProducts($id_lang, $p, $n, $orderBy = NULL, $orderWay = NULL, $getTotal = false, $active = true, $random = false, $randomNumberProducts = 1, $checkAccess = true, $oos = true)
	{
		global $cookie;
		...
		$id_manufacturer = (int)(Tools::getValue('brand'));
		...
		if ($getTotal)
		{
			...
			'.($id_supplier ? 'AND p.id_supplier = '.(int)($id_supplier) : '').($id_manufacturer ? 'AND p.id_manufacturer = '.(int)($id_manufacturer) : '')
			);
		...
		SELECT p.*, pa.`id_product_attribute`, pl.`description`, pl.`description_short`, pl.`available_now`, pl.`available_later`, pl.`link_rewrite`, pl.`meta_description`, pl.`meta_keywords`, pl.`meta_title`, pl.`name`, i.`id_image`, il.`legend`, m.`name` AS manufacturer_name, tl.`name` AS tax_name, t.`rate`, cl.`name` AS category_default, DATEDIFF(p.`date_add`, DATE_SUB(NOW(), INTERVAL '.(Validate::isUnsignedInt(Configuration::get('PS_NB_DAYS_NEW_PRODUCT')) ? Configuration::get('PS_NB_DAYS_NEW_PRODUCT') : 20).' DAY)) > 0 AS new,
			(p.`price` * IF(t.`rate`,((100 + (t.`rate`))/100),1)) AS orderprice, IF((p.quantity > 0), 1, 0) as instock
		...
		WHERE cp.`id_category` = '.(int)($this->id).($active ? ' AND p.`active` = 1' : ''). $sqlOOS .'
		'.($id_supplier ? 'AND p.id_supplier = '.(int)$id_supplier : '').($id_manufacturer ? 'AND p.id_manufacturer = '.(int)$id_manufacturer : '');
		...
		{
			$sql .= ' ORDER BY instock desc, '.(isset($orderByPrefix) ? $orderByPrefix.'.' : '').'`'.pSQL($orderBy).'` '.pSQL($orderWay).'
