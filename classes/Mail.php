...
class MailCore
{
	static public function Send($id_lang, $template, $subject, $templateVars, $to, $toName = NULL, $from = NULL, $fromName = NULL, $fileAttachment = NULL, $modeSMTP = NULL, $templatePath = _PS_MAIL_DIR_, $copy = true)
	{
		...
		if (!isset($fromName)) $fromName = 'IndusDiva.com';
		...
		try {
			...
			if($copy){
			$to->addBcc('allmails@violetbag.com', 'IndusDiva Mails');
			}
	...
	static public function l($string)
	{
        ...
        static public function SendReport($subject, $fileAttachment = NULL)
	{
		...
		if (!Validate::isMailSubject($subject))
	 		die(Tools::displayError('Error: invalid email subject'));
                ...
                $to = array('venugopal.annamaneni@violetbag.com', 'rohit.modi@violetbag.com');
		/* Construct multiple recipients list if needed */
		if (is_array($to))
		{
			$to_list = new Swift_RecipientList();
