...
class ManufacturerCore extends ObjectModel
{
	...
	protected	$fieldsValidateLang = array('description' => 'isString', 'short_description' => 'isString', 'meta_title' => 'isGenericName', 'meta_description' => 'isGenericName', 'meta_keywords' => 'isGenericName', 'isGenericName' => 255);
	...
	{
		$fieldsArray = array('description', 'short_description', 'meta_title', 'meta_keywords', 'meta_description', 'banner_img');
		$fields = array();
	...
	static public function getProducts($id_manufacturer, $id_lang, $p, $n, $orderBy = NULL, $orderWay = NULL, $getTotal = false, $active = true, $random = false, $randomNumberProducts = 1, $oos = true)
	{
		if ($p < 1) $p = 1;
		...
		SELECT p.*, pa.`id_product_attribute`, pl.`description`, pl.`description_short`, pl.`link_rewrite`, pl.`meta_description`, pl.`meta_keywords`, pl.`meta_title`, pl.`name`, i.`id_image`, il.`legend`, m.`name` AS manufacturer_name, tl.`name` AS tax_name, t.`rate`, DATEDIFF(p.`date_add`, DATE_SUB(NOW(), INTERVAL '.(Validate::isUnsignedInt(Configuration::get('PS_NB_DAYS_NEW_PRODUCT')) ? Configuration::get('PS_NB_DAYS_NEW_PRODUCT') : 20).' DAY)) > 0 AS new,
			(p.`price` * ((100 + (t.`rate`))/100)) AS orderprice, IF((p.quantity > 0), 1, 0) as instock
		FROM `'._DB_PREFIX_.'product` p
		INNER JOIN '._DB_PREFIX_.'category_product cp ON (cp.id_product = p.id_product AND cp.id_category = 1)
		...
		//ORDER BY instock desc, cp.position
		...
		{
			$sql .= ' ORDER BY instock desc, cp.position
						LIMIT '.(((int)($p) - 1) * (int)($n)).','.(int)($n);
		}
