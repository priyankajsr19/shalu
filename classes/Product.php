...
class ProductCore extends ObjectModel
{
    ...
    protected $fieldsSize = array('reference' => 32, 'supplier_reference' => 48, 'location' => 64, 'ean13' => 13, 'upc' => 12, 'unity' => 10);
    protected $fieldsValidate = array(
        'id_tax_rules_group' => 'isUnsignedId',
        'id_manufacturer' => 'isUnsignedId',
        'id_sizechart' => 'isUnsignedId',
    ...
    public function getFields()
    {
        ...
        $fields['is_customizable'] = (int)($this->is_customizable);
        $fields['shipping_sla'] = (int)($this->shipping_sla);
        $fields['as_shown'] = (int)($this->as_shown);
        $fields['is_rts'] = (int)($this->is_rts);
	$fields['has_bottom'] = (int)($this->has_bottom);
        $fields['is_exclusive'] = (int)($this->is_exclusive);
        $fields['is_plussize'] = (int)($this->is_plussize);
        $fields['id_sizechart'] = (int)($this->id_sizechart);
        $fields['cashback_percentage'] = (int)($this->cashback_percentage);
        return $fields;
    }
    ...
    */
    ...
    public function validateFieldsLang($die = true, $errorReturn = false)
    {
        ...
        foreach ($this->description_short as $k => $value)
            ...
            {
                if ($die) die (Tools::displayError().' ('.get_class($this).'->description: length > 4000 for language '.$k.')');
    ...
    */
    public static function getNewProducts($id_lang, $pageNumber = 0, $nbProducts = 10, $count = false, $orderBy = NULL, $orderWay = NULL,$random = false, $randomNumberProducts = 1)
    {
        ...
        {
            $sql .= 'ORDER BY '.(isset($orderByPrefix) ? pSQL($orderByPrefix).'.' : '').'`'.pSQL($orderBy).'` '.pSQL($orderWay).'
    ...
    * @return array Special
    */
    public function getLeafCategory($id_lang)
    {
        $leafCategory = false;
        $minLevel = 100;
        ...
        {
            $cat = new Category($categoryID, $id_lang);
            if($cat->level_depth < $minLevel)
            {
                $maxLevel = $cat->level_depth;
                $leafCategory = $cat;
            }
        }
    ...
    */
    ...
    $usereduc = true, $quantity = 1, $forceAssociatedTax = false, $id_customer = NULL, $id_cart = NULL, $id_address = NULL, &$specificPriceOutput = NULL, $with_ecotax = TRUE, $id_affiliate = NULL)
    {
           global $cookie, $cart;
        $cur_cart = $cart;
        ...
        if (!is_object($cur_cart) AND (Validate::isUnsignedInt($id_cart) AND $id_cart))
        {
            /*
        ...
        $usereduc, $with_ecotax, $specificPriceOutput, $id_affiliate);
    }
    ...
    */
    ...
    public function getAffiliatePrice($id_affiliate = null, $tax = true, $id_product_attribute = NULL, $decimals = 6, $divisor = NULL, $only_reduc = false, $usereduc = true, $quantity = 1) {
        $temp = null;
        return self::getPriceStatic((int)($this->id), $tax, $id_product_attribute, $decimals, $divisor, $only_reduc, $usereduc, $quantity, false, null, null, null, $temp ,true, $id_affiliate);
    }
    ...
    */
    ...
    static function convertAndShow($params, &$smarty)
    {
        global $currency;
        return Tools::displayPrice(Tools::convertPrice($params['price'], $currency), $currency, false);
    }
    ...
    */
    ...
    public static function getProductProperties($id_lang, $row)
    {
        ...
        //check in cache before returning
        $cacheHit = false;
        
        //$cacheRow = apc_fetch($cacheKey, $cacheHit);
        //if($cacheHit)
            //return $cacheRow;
    ...
    */
    ...
    public static function getAllCustomizedDatas($id_cart, $id_lang = null)
    {
        ...
        if (!$result = Db::getInstance()->ExecuteS('
            SELECT cd.`id_customization`, c.`id_product`, c.`customer_height`,cfl.`id_customization_field`, c.`id_product_attribute`, cd.`type`, cd.`index`, cd.`value`, cfl.`name`
        ...
        foreach ($result AS $row)
        {
            ...
            if($row['index'] == 1)
            {
                $saree_customization = explode("_",$row['value']);
                $fall_piko = 1;
                if( count($saree_customization) > 1 )
                    $fall_piko = $saree_customization[1];
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['fall_piko'] = $fall_piko;
            }
            elseif($row['index'] == 2)
            {
                $blouse_customization = explode('_', $row['value']);
                $id_measurement = $blouse_customization[0];
                $id_style =  $blouse_customization[1];
                $fall_piko = 1;
                if( count($blouse_customization) > 2 )
                    $fall_piko = $blouse_customization[2];
                    
                $res = Db::getInstance()->getRow("SELECT m.* FROM ps_blouse_measurements m WHERE id_measurement = " . $id_measurement);
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['blouse_measurement'] = $res['name'];
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['blouse_measurement_id'] = $id_measurement;
                    
                $res = Db::getInstance()->getRow("SELECT style_name, style_image FROM ps_styles WHERE style_type = 1 AND id_style = " . $id_style);
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['blouse_style_name'] = $res['style_name'];
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['blouse_style_image'] = $res['style_image'];
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['fall_piko'] = $fall_piko;
            }
            else if($row['index'] == 3)
            {
                $inskirt_customization = explode('_', $row['value']);
                $id_measurement = $inskirt_customization[0];
                $id_style = $inskirt_customization[1];
                $fall_piko = 1;
                if( count($inskirt_customization) > 2 )
                    $fall_piko = $inskirt_customization[2];
                    
                $res = Db::getInstance()->getRow("SELECT m.* FROM ps_skirt_measurements m WHERE id_measurement = " . $id_measurement);
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['inskirt_measurement'] = $res['name'];
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['inskirt_measurement_id'] = $id_measurement;
                    
                $res = Db::getInstance()->getRow("SELECT style_name, style_image FROM ps_styles WHERE style_type = 2 AND id_style = " . $id_style);
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['inskirt_style_name'] = $res['style_name'];
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['inskirt_style_image'] = $res['style_image'];
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['fall_piko'] = $fall_piko;
            }
            elseif($row['index'] == 8)
            {
                ...
                if( count($saree_customization) > 1 )
                    $fall_piko = $saree_customization[1];
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['fall_piko'] = $fall_piko;
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['pre_stitched'] = "Standard Saree";
            }
            else if($row['index'] == 4)
            {
                $skd_customization = explode('_', $row['value']);
                $id_kurta_measurement = $skd_customization[0];
                $id_salwar_measurement = $skd_customization[1];
                    
                $res = Db::getInstance()->getRow("SELECT m.* FROM ps_kurta_measurements m WHERE id_measurement = " . $id_kurta_measurement);
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['kurta_measurement'] = $res['name'];
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['kurta_measurement_id'] = $id_kurta_measurement;
                
                $res = Db::getInstance()->getRow("SELECT m.* FROM ps_salwar_measurements m WHERE id_measurement = " . $id_salwar_measurement);
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['salwar_measurement'] = $res['name'];
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['salwar_measurement_id'] = $id_salwar_measurement;
            }
            else if($row['index'] == 5)
            {
                $lehengaCustomization = explode('_', $row['value']);
                $id_choli_measurement = $lehengaCustomization[0];
                $id_choli_style = $lehengaCustomization[1];
                $id_lehenga_measurement = $lehengaCustomization[2];
                $id_lehenga_style = $lehengaCustomization[3];
                
                $res = Db::getInstance()->getRow("SELECT m.* FROM ps_blouse_measurements m WHERE id_measurement = " . $id_choli_measurement);
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['choli_measurement'] = $res['name'];
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['choli_measurement_id'] = $id_choli_measurement;
                    
                $res = Db::getInstance()->getRow("SELECT style_name, style_image_small FROM ps_styles WHERE style_type = 3 AND id_style = " . $id_choli_style);
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['choli_style_name'] = $res['style_name'];
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['choli_style_image'] = $res['style_image_small'];
                    
                $res = Db::getInstance()->getRow("SELECT m.* FROM ps_skirt_measurements m WHERE id_measurement = " . $id_lehenga_measurement);
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['lehenga_measurement'] = $res['name'];
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['lehenga_measurement_id'] = $id_lehenga_measurement;
                    
                $res = Db::getInstance()->getRow("SELECT style_name, style_image_small FROM ps_styles WHERE style_type = 2 AND id_style = " . $id_lehenga_style);
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['lehenga_style_name'] = $res['style_name'];
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['lehenga_style_image'] = $res['style_image_small'];
            }
            else if($row['index'] == 13)
            {
                $choliCustomization = explode('_', $row['value']);
                $choliStyleID = $choliCustomization[0];
                $choliSize = $choliCustomization[1];
                $choliStyle = '';
                
                if($choliStyleID == 1) $choliStyle = "Designer";
                elseif ($choliStyleID == 2) $choliStyle = "Traditional Indian Cut";
                else $choliStyle = "Corset";
                
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['choli_style'] = $choliStyle;
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['choli_size'] = $choliSize;
            }
            else if($row['index'] == 21)
            {
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['friends_name'] = $row['value'];
            }
            else if($row['index'] == 22)
            {
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['friends_email'] = $row['value'];
            }
            else if($row['index'] == 23)
            {
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['gift_message'] = $row['value'];
            }
            else if($row['index'] == 25)
            {
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['voucher_code'] = $row['value'];
            }
            else if($row['index'] == 24)
            {
                $lehengaCustomization = explode('_', $row['value']);
                $id_kurta_measurement = $lehengaCustomization[0];
                $id_kurta_style = $lehengaCustomization[1];
                $id_salwar_measurement = $lehengaCustomization[2];
                $id_salwar_style = $lehengaCustomization[3];
                
                if( $id_kurta_measurement !== 'NA' )
                    $res = Db::getInstance()->getRow("SELECT m.* FROM ps_kurta_measurements m WHERE id_measurement = " . $id_kurta_measurement);
                else
                    $res['name'] = null;
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['kurta_measurement'] = $res['name'];
                ...
                if( $id_kurta_style !== 'NA' )    
                    $res = Db::getInstance()->getRow("SELECT style_name, style_image_small FROM ps_styles WHERE style_type = 4 AND id_style = " . $id_kurta_style);
                else {
                    $res['style_name'] = null;
                    $res['style_image_small'] = null;
                }
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['kurta_style_name'] = $res['style_name'];
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['kurta_style_image'] = $res['style_image_small'];
            

                if( $id_salwar_measurement != 'NA' )        
                    $res = Db::getInstance()->getRow("SELECT m.* FROM ps_salwar_measurements m WHERE id_measurement = " . $id_salwar_measurement);
                else
                    $res['name'] = null;
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['salwar_measurement'] = $res['name'];
                ...
                if( $id_salwar_style !== 'NA' )
                    $res = Db::getInstance()->getRow("SELECT style_name, style_image_small FROM ps_styles WHERE style_type = 5 AND id_style = " . $id_salwar_style);    
                else {
                    $res['style_name'] = null;
                    $res['style_image_small'] = null;
                }
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['salwar_style_name'] = $res['style_name'];
                $customizedDatas[(int)($row['id_product'])][(int)($row['id_product_attribute'])][(int)($row['id_customization'])]['salwar_style_image'] = $res['style_image_small'];
            }
        }
    ...
    */
    ...
    public function getShadeDetails()
    {
        return Db::getInstance()->ExecuteS('
        SELECT s.id_product, s.color, s.name, s.texture
        FROM `'._DB_PREFIX_.'shades` s inner join ps_product p on (p.id_product = s.id_product)
        WHERE s.`id_group` = '.(int)($this->id_group).' and p.active = 1');
    }
    ...
    public function getRelatedProducts()
    {
        return Db::getInstance()->ExecuteS('
                SELECT id_product
                FROM `'._DB_PREFIX_.'product` where `id_group` = '.(int)($this->id_group).' and id_product <> ' . $this->id . ' and active = 1');
    }


    public function updateTags($tags){
        foreach ($tags as $tag) {
            $result = Db::getInstance()->ExecuteS('SELECT `id_tag` FROM `'._DB_PREFIX_.'tag` WHERE `name` = "'.$tag . '" ');
            $tag_id = $result[0]['id_tag'];
            $result = Db::getInstance()->ExecuteS('SELECT * FROM `'._DB_PREFIX_.'product_tag` WHERE `id_tag` = '.(int)$tag_id .' AND `id_product`='.(int)$this->id);
            if(!$result){
                $sql = 'INSERT INTO `'._DB_PREFIX_.'product_tag` VALUES ('.$this->id.', '.(int)$tag_id.')';
                Db::getInstance()->Execute($sql);die;
            }
        }
    }
    ...
    public static function isProductOfCategory($id_product, $id_category) {
        $sql = "select count(*) as count from ps_product p join ps_category_product cp on p.id_product = cp.id_product and cp.id_category = $id_category and p.id_product = $id_product";
        $row = Db::getInstance()->getRow($sql);
        $count = (int)$row['count'];
        return ($count)?true:false;
    }

    public static function getCBPercentageValue($id_product) {
	$sql = "select cashback_percentage from ps_product where id_product = ".$id_product;
	$cb = Db::getInstance()->getRow($sql);
	return (int)$cb['cashback_percentage'];
    }
}
