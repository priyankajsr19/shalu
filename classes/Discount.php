...
class DiscountCore extends ObjectModel
{
	...
	protected static $_discountCategories = array();
	...
	protected	$webserviceParameters = array(
			...
			'brands' => array('sqlId' => 'brands'),
			'min_cart' => array('sqlId' => 'min_cart'),
			'spl_voucher' => array('sqlId' => 'spl_voucher')
		)
	);
	...
	public function getValue($nb_discounts = 0, $order_total_products = 0, $shipping_fees = 0, $idCart = false, $useTax = true)
	{
		...
		switch ($this->id_discount_type)
		{
			...
			case 4:
				...
				$taxDiscount = $cart->getTaxesAverageUsedByObj();
				...
				$value = Tools::convertPrice($value, $cart->id_currency, true);
				return $value;
    ...
    public function getValueByCartObj($nb_discounts = 0, $order_total_products = 0, $shipping_fees = 0, $cart = false, $useTax = true)
    {
    	//see if global cart is set
    	$totalAmount = 0;
	...
	{
		if(!array_key_exists($id_discount, self::$_discountCategories))
		{		
			self::$_discountCategories[$id_discount] =  Db::getInstance()->ExecuteS('
			SELECT `id_category`
			FROM `'._DB_PREFIX_.'discount_category`
			WHERE `id_discount` = '.(int)($id_discount));
		}
		return self::$_discountCategories[$id_discount];
	}
	...
	static public function createOrderDiscount($order, $productList, $qtyList, $name, $shipping_cost = false, $id_category = 0, $subcategory = 0)
	{
		...
		{
			if ($discount['id_discount_type'] == 1 || $discount['id_discount_type'] == 5)
				$total -= $total_tmp * ($discount['value'] / 100);
			elseif ($discount['id_discount_type'] == 2 || $discount['id_discount_type'] == 4)
	...
	static public function getVouchersToCartDisplay($id_lang, $id_customer)
	{
		...
		SELECT d.`name`, dl.`description`, d.`id_discount`,d.`minimal`
		FROM `'._DB_PREFIX_.'discount` d
