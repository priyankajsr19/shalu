...
/*
	...
	** @return array from Product::getProductProperties
	*/
	static public function getBestSalesManufacturer($id_lang, $id_manufacturer, $pageNumber = 0, $nbProducts = 10, $orderBy=NULL, $orderWay=NULL, $oos = true)
	{
		if ($pageNumber < 0) $pageNumber = 0;
		if ($nbProducts < 1) $nbProducts = 10;
		if (empty($orderBy) || $orderBy == 'position') $orderBy = 'sales';
		...
		AND p.id_manufacturer = ' . $id_manufacturer . $sqlOOS .'
		ORDER BY '.(isset($orderByPrefix) ? $orderByPrefix.'.' : '').'`'.pSQL($orderBy).'` '.pSQL($orderWay).'
		LIMIT '.(int)($pageNumber * $nbProducts).', '.(int)($nbProducts));
