...
class OrderCore extends ObjectModel
{
	...
	/** @var float cod total */
	public 		$total_cod;
	public 		$total_customization;
	public 		$expected_shipping_date;
        public 		$actual_expected_shipping_date;
	...
	public function getFields()
	{
		...
		$fields['total_donation'] = (float)($this->total_donation);
		...
		$fields['total_cod'] = (float)($this->total_cod);
		$fields['total_customization'] = (float)($this->total_customization);
		$fields['expected_shipping_date'] = pSQL($this->expected_shipping_date);
		$fields['actual_expected_shipping_date'] = pSQL($this->actual_expected_shipping_date);
		return $fields;
	}
	...
	protected function _deleteProduct($orderDetail, $quantity)
	{
		...
		$productPriceWithoutTax = number_format($quantity * $productPriceWithoutTax, 2, '.', '');
	...
	public function deleteCustomization($id_customization, $quantity, $orderDetail)
	{
		...
		elseif ($this->hasBeenPaid())
		{
			if(!Db::getInstance()->Execute('UPDATE `'._DB_PREFIX_.'customization` SET `quantity_refunded` = `quantity_refunded` + '.(int)($quantity).' WHERE `id_customization` = '.(int)($id_customization).' AND `id_cart` = '.(int)($this->id_cart).' AND `id_product` = '.(int)($orderDetail->product_id)))
				return false;
			return $this->_deleteProduct($orderDetail, (int)($quantity));
		}
	...
	public function setProductPrices(&$row)
	{
		if ($this->_taxCalculationMethod == PS_TAX_EXC)
			$row['product_price'] = Tools::ps_round($row['product_price']);
		else
			$row['product_price_wt'] = Tools::ps_round($row['product_price'] * (1 + $row['tax_rate'] / 100));
		...
		if ($row['reduction_amount'])
		{
			if ($this->_taxCalculationMethod == PS_TAX_EXC)
				...
				$row['product_price_wt'] = Tools::ps_round($row['product_price_wt'] - $row['reduction_amount'] * $group_reduction);
		}
		...
		{
			$row['product_price_wt_but_ecotax'] = Tools::ps_round($row['product_price_wt']);
			$row['product_price_wt'] = Tools::ps_round($row['product_price_wt'] + $row['ecotax'] * (1 + $row['ecotax_tax_rate'] / 100));
		}

		$row['total_wt'] = $row['product_quantity'] * Tools::ps_round($row['product_price_wt']);
		$row['total_price'] = $row['product_quantity'] * Tools::ps_round($row['product_price']);
	}
	...
	*/
	...
	static public function getDiscountsGroupCustomer($id_customer, $id_discount_group)
	{
		return Db::getInstance()->getValue('
			SELECT COUNT(*) FROM `'._DB_PREFIX_.'orders` o
			LEFT JOIN '._DB_PREFIX_.'order_discount od ON (od.id_order = o.id_order)
			INNER JOIN '._DB_PREFIX_.'discount d ON (od.id_discount = d.id_discount)
			WHERE o.id_customer = '.(int)($id_customer).'
			AND d.discount_group = '.(int)($id_discount_group));
	}
	...
	static public function getCustomerOrders($id_customer, $showHiddenStatus = false)
    {
		...
		foreach ($res AS $key => $val)
		{
			...
			SELECT os.`id_order_state`, osl.`name` AS order_state, osl.`state_name_external` AS order_state_external, os.`invoice`
			FROM `'._DB_PREFIX_.'order_history` oh
			LEFT JOIN `'._DB_PREFIX_.'order_state` os ON (os.`id_order_state` = oh.`id_order_state`)
	...
	*/
	...
	public function setInvoice()
	{
		...
		if ($number)
 		...
 		//append today's date to invoice number
 		$resNumber = Db::getInstance()->getRow('SELECT MAX(`invoice_number`) + 1 AS `invoice_number` FROM `'._DB_PREFIX_.'orders`', 0);
		$maxInvoiceNumber = $resNumber['invoice_number'];
		if(substr($maxInvoiceNumber, 0, 6) != date('ymd'))
			$maxInvoiceNumber = date('ymd').'0001';
 		
 		// a way to avoid duplicate invoice number
		Db::getInstance()->Execute('
		UPDATE `'._DB_PREFIX_.'orders`
		SET `invoice_number` = '.$maxInvoiceNumber.', `invoice_date` = \''.date('Y-m-d H:i:s').'\'
		WHERE `id_order` = '.(int)$this->id
		);
