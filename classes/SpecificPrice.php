...
class SpecificPriceCore extends ObjectModel
{
   ...
   // score generation for quantity discount
	...
	static public function getSpecificPrice($id_product, $id_shop, $id_currency, $id_country, $id_group, $quantity, $id_affiliate)
	{
            if( $id_affiliate != null ) {
		$key = ((int)$id_product.'-'.(int)$id_shop.'-'.(int)$id_currency.'-'.(int)$id_country.'-'.(int)$id_group.'-'.(int)$quantity).'-'.(int)$id_affiliate;
		if (!array_key_exists($key, self::$_specificPriceCache))
		{
                    ...
                    $res = Db::getInstance()->getRow('
                        ...
                        FROM `'._DB_PREFIX_.'affiliate_specific_price`
                        WHERE `id_product` IN (0, '.(int)$id_product.')
                        AND `id_affiliate` IN (0, '.(int)$id_affiliate.')
