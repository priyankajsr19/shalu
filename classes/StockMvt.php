...
class StockMvtCore extends ObjectModel
{
	...
	public function add($autodate = true, $nullValues = false, $update_quantity = true)
	{
        ...
        //update date_upd on quantity update
        $date_upd = date("Y-m-d H:i:s");
		if ($this->id_product_attribute)
		{
            $product = new Product((int)($this->id_product), false, Configuration::get('PS_LANG_DEFAULT'));
            
			Db::getInstance()->Execute("UPDATE "._DB_PREFIX_."product SET date_upd = '".$date_upd."' WHERE id_product=".(int)$this->id_product);
			...
			return Db::getInstance()->Execute("UPDATE "._DB_PREFIX_."product SET date_upd = '".$date_upd."', quantity=quantity+".(int)$this->quantity." WHERE id_product=".(int)$this->id_product);
	}
