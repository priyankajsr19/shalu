...
class CustomerCore extends ObjectModel {
    ...
    /** @var string Object facebook userid */
    public $fbid;

    /** @var string Object facebook access token */
    public $fbtoken;
    public $years;
    public $days;
    public $months;
    ...
    protected $fieldsSize = array('lastname' => 32, 'passwd' => 32, 'firstname' => 32, 'email' => 128, 'note' => 65000, 'fbid' => 80, 'fbtoken' => 256);
    protected $fieldsValidate = array('unsubscribe'=> 'isMd5', 'secure_key' => 'isMd5', 'lastname' => 'isName', 'firstname' => 'isName', 'email' => 'isEmail', 'passwd' => 'isPasswd',
    ...
    public function getFields() {
        ...
        $fields['fbid'] = pSQL($this->fbid);
        $fields['fbtoken'] = pSQL($this->fbtoken);
        $fields['id_referrer'] = pSQL($this->id_referrer);
        $fields['total_delivered'] = pSQL($this->total_delivered);
    ...
    public function add($autodate = true, $nullValues = true) {
        ...
        $this->unsubscribe = md5(uniqid(rand(), true));
        $this->last_passwd_gen = date('Y-m-d H:i:s', strtotime('-' . Configuration::get('PS_PASSWD_TIME_FRONT') . 'minutes'));

        $this->id_referrer = self::getReferrer($this->email);
        $this->id_country = $this->id_country ? $this->id_country : Configuration::get('PS_COUNTRY_DEFAULT');
    ...
    public function isInternal() {
        $groups = self::getGroupsStatic((int) ($this->id));
        return in_array(2, $groups);
    }
    ...
    public static function getReferrer($email) {
        global $cookie;
        if ($cookie->last_ref_source)
            return $cookie->last_ref_source;

        $result = Db::getInstance()->getRow("select id_customer from vb_customer_referrals
				where email = '" . $email . "'
				order by id desc");
        if (!$result)
            return false;
    ...
    public static function getTotalDeliveredForCustomer($id_customer) {
        $result = Db::getInstance()->getRow("select count(*) as 'total_delivered'
					from ps_orders o
					...
					and oh.`id_order_state` = " . _PS_OS_DELIVERED_);
    ...
    public static function getTotalApprovedReviews($id_customer) {
        $result = Db::getInstance()->getRow("select count(*) as 'total_approved'
        ...
        return (int) $result['total_approved'];
    }

    public static function getCustomerReferrals($id_customer) {
        $result = Db::getInstance()->ExecuteS("select id_customer, firstname, lastname, email, total_delivered from ps_customer where id_referrer = " . $id_customer);
    ...
    public static function getValidOrders($id_customer) {
        $sql = "select count(*) as valid_orders from ps_orders where valid = 1 and id_customer = {$id_customer}";
        $res = Db::getInstance()->ExecuteS($sql);
        return $res[0]["valid_orders"];
    }

    public static function getCustomerByUnSubscribeKey($unsub_key) {
        $sql = "select id_customer from ps_customer where unsubscribe = '$unsub_key'";
