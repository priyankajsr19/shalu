...
if(Tools::getValue('getSalesData'))
{
    ...
    if(Tools::isSubmit('download-orders-invoice'))
    {
        ...
        $header = array('Date Order', 'invoice ID', 'Order ID', 'Cart ID', 'Payment Method', 'Total Pretax', 'Discount', 'Tax', 'Shipping', 'Order Total', 'Total Paid', 'Conversion Rate', 'USD to INR', 'CurrentState', 'Country', 'City', 'Carrier','Promised Shipping Date', 'Expected Shipping Date' ,'Email','CustomerName','NumProducts','Address1','Address2','City','Postcode','Mobile');
    }
    ...
    if(Tools::isSubmit('download-orders-delivery'))
    {
        ...
        $header = array('Date Order', 'Date Delivery', 'Promised Shipping Date' , 'Date Shipped', 'invoice ID', 'Order ID', 'Cart ID', 'Payment Method', 'AWB','Total Pretax', 'Discount', 'Tax', 'Shipping', 'Order Total', 'Total Paid', 'Conversion Rate', 'CurrentState', 'Ship Country', 'Bill Country', 'City', 'Carrier','Email');
    }
...
if (Tools::getValue('getSearchKeyword'))
{
    $date_from = Tools::getValue('search_date_from');
    $date_to = Tools::getValue('search_date_to'); 
    //searched Keyword

    $sql_search_keyword = "SELECT keywords , count(keywords) as total from ps_statssearch where date_add BETWEEN '" . $date_from ."' AND '" . $date_to ."' GROUP BY keywords";

      if(Tools::isSubmit('download-searched-keyword'))
    {
        $filename = 'indusdiva-searched-keyword-'.$date_from.'-to-'.$date_to.'.csv';
        $sql = $sql_search_keyword;
        $header = array('keywords', 'total');
    }  
    ...
    if(Tools::isSubmit('download-searched-keyword'))
        foreach($res as $row){
        $record = array($row['keywords'], $row['total']);
        fputcsv($outstream, $record, ',', '"');
    }
    fclose($outstream);
}
...
IF(a.payment = 'EBS', NULL, round(a.total_paid)) 
as `COD value`, psc.name as `carrier`, coalesce(a.shipping_number, '') as 'tracking_no', round(a.total_paid) as 'order value'
FROM `ps_orders` a 
