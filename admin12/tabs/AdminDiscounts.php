...
class AdminDiscounts extends AdminTab
{
	public function __construct()
	{
		...
		IF(a.id_discount_type = 4, CONCAT(a.value, " ", c.sign),
		"--"))) as strvalue';
	...
	public function postProcess()
	{
		...
		if (Tools::getValue('submitAdd'.$this->table))
		{
			...
			if ((Tools::getValue('id_discount_type') == 2 || Tools::getValue('id_discount_type') == 4) AND Tools::getValue('id_currency') == 0)
				$this->_errors[] = Tools::displayError('Please set a currency for this voucher.');			
			if ((Tools::getValue('id_discount_type') == 1 || Tools::getValue('id_discount_type') == 2 || Tools::getValue('id_discount_type') == 4) && !Tools::getValue('value'))
	...
	public function displayForm($isMainTab = true)
	{
		...
		<script type="text/javascript">
			function discountType()
			{
				...
				else if ($("#id_discount_type").val() == 1 || $("#id_discount_type").val() == 5)
				{
					$("#value-div").css("display", "block");
					$("#percent-span").css("display", "block");
					$("#id_currency").css("display", "none");
				}
				else if ($("#id_discount_type").val() == 2 || $("#id_discount_type").val() == 4)
				{
					$("#value-div").css("display", "block");
					...
					$("#value-div").css("display", "none");
				if ($(\'#id_discount_type\').val() != 2 && $(\'#id_discount_type\').val() != 4)
		...
		echo '		
				...
				<div class="margin-form">
					<label class="t" for="brands">Brand IDs:</label>
					<input type="text" name="brands" id="brands" value="'. ($this->getFieldValue($obj, 'brands') ? $this->getFieldValue($obj, 'brands') : '') .'" />
					<label class="t" for="min_cart">Minimum Cart Value:</label>
					<input type="text" name="min_cart" id="min_cart" value="'. ($this->getFieldValue($obj, 'min_cart') ? $this->getFieldValue($obj, 'min_cart') : '') .'" />
				</div>
				<div class="margin-form">
