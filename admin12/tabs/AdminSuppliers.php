...
class AdminSuppliers extends AdminTab
{
	...
	public function __construct()
	{
		...
		$this->fieldsDisplay = array(
			...
			'code' => array('title' => $this->l('Code'), 'width' => 120),
	...
	public function displayForm($isMainTab = true)
	{
		...
		'.($supplier->id ? '<input type="hidden" name="id_'.$this->table.'" value="'.$supplier->id.'" />' : '').'
			<fieldset>
                            <legend><img src="../img/admin/suppliers.gif" />'.$this->l('Suppliers').'</legend>
				...
				<div class="margin-form">
					<input type="text" size="40" name="code" value="'.htmlentities(Tools::getValue('code', $supplier->code), ENT_COMPAT, 'UTF-8').'" /> <sup>*</sup>
					<span class="hint" name="help_box">'.$this->l('Invalid characters:').' <>;=#{}<span class="hint-pointer">&nbsp;</span></span>
				</div>
                                
				<label>'.$this->l('Location:').' </label>';
                $locations = Supplier::getSupplierLocations();
                
		echo		'<div class="margin-form">
                                        <select name="id_location">
                                            <option value="">Select..</option>';
                                        $id_location = Tools::getValue('id_location', $supplier->id_location);
                                        foreach($locations as $loc) {
                                            if( (string)$id_location === (string)$loc['id_location'])
                                                echo "<option value='".$loc['id_location']."' selected>".$loc['name']."</option>";
                                            else
                                                echo "<option value='".$loc['id_location']."' >".$loc['name']."</option>";
                                        }
                echo                    '</select>
					<span class="hint" name="help_box">'.$this->l('Invalid characters:').' <>;=#{}<span class="hint-pointer">&nbsp;</span></span>
				</div>

				<label>'.$this->l('Contact Person:').' </label>
				<div class="margin-form">
					<input type="text" size="40" name="contact_person" value="'.htmlentities(Tools::getValue('contact_person', $supplier->contact_person), ENT_COMPAT, 'UTF-8').'" />
					<span class="hint" name="help_box">'.$this->l('Invalid characters:').' <>;=#{}<span class="hint-pointer">&nbsp;</span></span>
				</div>
				...
				<div class="margin-form">
					<input type="text" size="40" name="phone" value="'.htmlentities(Tools::getValue('phone', $supplier->phone), ENT_COMPAT, 'UTF-8').'" />
					<span class="hint" name="help_box">'.$this->l('Invalid characters:').' <>;=#{}<span class="hint-pointer">&nbsp;</span></span>
				</div>

				<label>'.$this->l('Email:').' </label>
				<div class="margin-form">
					<input type="text" size="40" name="email" value="'.htmlentities(Tools::getValue('email', $supplier->email), ENT_COMPAT, 'UTF-8').'" />
					<span class="hint" name="help_box">'.$this->l('Invalid characters:').' <>;=#{}<span class="hint-pointer">&nbsp;</span></span>
				</div>
				...
				<div class="margin-form">
					<textarea cols="40" row="6" name="address" value="'.htmlentities(Tools::getValue('address', $supplier->address), ENT_COMPAT, 'UTF-8').'"></textarea>
					<span class="hint" name="help_box">'.$this->l('Invalid characters:').' <>;=#{}<span class="hint-pointer">&nbsp;</span></span>
				</div>
