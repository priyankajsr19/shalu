...
var ajaxCart = {
	...
	overrideButtonsInThePage : function(){
		...
		$('.ajax_add_to_cart_button').unbind('click').click(function(){
			var idProduct =  $(this).attr('idprod').replace('ajax_id_product_', '');
			if ($(this).attr('disabled') != 'disabled')
				ajaxCart.add(idProduct, null, false, this);
			return false;
		});
		//for product page 'add' button...
		/*$('body#product input.addtobag').unbind('click').click(function(){
	...
	add : function(idProduct, idCombination, addedFromProductPage, callerElement, quantity, whishlist){
		...
		//send the ajax request to the server
		//customization datas
		$custData = '&prestitched_saree=' + $('#prestitched_saree').val();
		$custData = $custData + '&prestitched_blouse=' + $('#prestitched_blouse').val();
		$custData = $custData + '&prestitched_inskirt=' + $('#prestitched_inskirt').val();
		$custData = $custData + '&measurement_id=' + $('#measurement_id').val();
		$custData = $custData + '&blouse_style_id=' + $('#blouse_style_id').val();
		$.ajax({
			type: 'POST',
			url: baseDir + 'cart.php',
			async: true,
			cache: false,
			dataType : "json",
			data: 'add=1&ajax=true&qty=' + ((quantity && quantity != null) ? quantity : '1') 
			...
			success: function(jsonData,textStatus,jqXHR)
			{
				...
				var $element = $(callerElement).parent().parent().find('div.product_image img,div.product_img_link img');
				if (!$element.length)
					$element = $('#bigpic');
				...
				var cartBlockOffset = $('#shopping-bag').offset();
				...
				$.fancybox.open({
					type: 'ajax',
	        		href:	baseDir +'cart-summary-large.php?summary=1&ajax=1',
	        		ajax: true,
	        		afterLoad: function(){
	        			console.log('called');
		...
		$(jsonData.products).each(function(){
			...
			if (this.id != undefined)
			{
				...
				if ($('div#cart_block dl.products').length == 0)
					$('p#cart_block_no_products').css('display','none').before('<dl class="products"></dl>');
	...
	updateCartEverywhere : function(jsonData) {
		...
		$('.ajax_products_total').text(jsonData.only_products);
		$('.ajax_cart_shipping_cost').text("Extra");
		$('.ajax_cart_discounts').text(jsonData.totalDiscounts);
		
		//hide shipping cost
		if(jsonData.shippingCost == 'Rs. 0')
		{
			$('#rht_bar_shipping').text('FREE(India)');
		}
		...
		if(parseInt(jsonData.nbTotalProducts) > 0)
		{
			...
			$('.lnk_shopping_bag').addClass('enabled');
		...
		else
		{
			...
			//more no products handling
			$('.lnk_shopping_bag').removeClass('enabled');
			$('#button_order_cart').fadeOut('slow');
		}
	}
};
...
$(document).ready(function(){
	$('.lnk_shopping_bag').click(function(e) {
