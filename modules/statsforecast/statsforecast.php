...
class StatsForecast extends Module
{
	...
	public function hookAdminStatsModules()
	{
		...
		SELECT
			...
			SUM(od.product_price * od.product_quantity * IFNULL(pr.conversion_rate, 55) / o.conversion_rate) as totalProducts,
			SUM(p.wholesale_price * od.product_quantity * 55) as totalSourcing,
  			IFNULL(pr.conversion_rate, 55) as dailyRate
		FROM '._DB_PREFIX_.'orders o
		LEFT JOIN '._DB_PREFIX_.'order_detail od ON o.id_order = od.id_order
        LEFT JOIN '._DB_PREFIX_.'product p ON od.product_id = p.id_product
        LEFT JOIN '._DB_PREFIX_.'currency_rates pr ON date(pr.day) = date(o.invoice_date)
		WHERE o.valid = 1
		...
		$this->_html .= '<div style="float:left;width:965px">
		<fieldset><legend><img src="../modules/'.$this->name.'/logo.gif" /> '.$this->displayName.'</legend>
			...
			<table class="table" border="0" cellspacing="0" cellspacing="0">
				<tr>
					...
					<th style="text-align:center;width:50px;">'.$this->l('% Reg.').'</th>
					<th style="text-align:center;width:70px;">'.$this->l('% Orders').'</th>
					<th style="width:80px;text-align:center">'.$this->l('Coupons').'</th>
					<th style="width:100px;text-align:center">'.$this->l('Sourcing Total').'</th>
					<th style="width:100px;text-align:center">'.$this->l('Pre-tax Total').'</th>
					<th style="width:70px;text-align:center">'.$this->l('Tax').'</th>
					<th style="width:100px;text-align:center">'.$this->l('Shipping').'</th>
					<th style="width:100px;text-align:center">'.$this->l('Cust.').'</th>
					<th style="width:100px;text-align:center">'.$this->l('Coins.').'</th>
					<th style="width:100px;text-align:center">'.$this->l('Order Total').'</th>
				</tr>';
		...
		SELECT '.$dateFromGInvoice.' as fix_date, SUM(od.value * IFNULL(pr.conversion_rate,55)/ o.conversion_rate) as total
		FROM '._DB_PREFIX_.'orders o
		...
		$points = Db::getInstance()->ExecuteS('
		        SELECT '.$dateFromGInvoice.' as fix_date, SUM(cp.points) as total_points
		        FROM '._DB_PREFIX_.'orders o
		        LEFT JOIN vb_cart_points cp ON o.id_cart = cp.id_cart
		        WHERE o.valid = 1
		        AND o.invoice_date BETWEEN '.ModuleGraph::getDateBetween().'
		        GROUP BY '.$dateFromGInvoice, false);
		while ($row = $db->nextRow($points))
		    $pointsArray[$row['fix_date']] = $row['total_points'];
		
		$today = date('Y-m-d');
		
		$ordersArray = array();
		$orders = Db::getInstance()->ExecuteS('
		SELECT '.$dateFromGInvoice.' as fix_date, SUM(o.`total_products` * IFNULL(pr.conversion_rate,55)/ o.conversion_rate) as totalPreTax, 
			SUM((o.`total_products_wt` - o.`total_products`) * IFNULL(pr.conversion_rate,55)/ o.conversion_rate) as Tax, 
			SUM((o.`total_shipping` + COALESCE(o.total_cod, 0))  * IFNULL(pr.conversion_rate,55)/ o.conversion_rate) as Shipping,
			SUM((COALESCE(o.total_customization, 0)) * IFNULL(pr.conversion_rate,55)/ o.conversion_rate) as Customization, 
			SUM((o.`total_products_wt` + o.`total_shipping` + COALESCE(o.total_cod, 0)) * IFNULL(pr.conversion_rate,55)/ o.conversion_rate) as `orderTotal`
            FROM '._DB_PREFIX_.'orders o
            LEFT JOIN '._DB_PREFIX_.'currency_rates pr ON date(pr.day) = date(o.invoice_date)
		WHERE o.valid = 1
		...
		AND o.invoice_date BETWEEN '.ModuleGraph::getDateBetween().'
		GROUP BY '.$dateFromGInvoice, false);
		while ($row = $db->nextRow($orders))
		{
			$ordersArray[$row['fix_date']] = $row;
		}
		
		foreach ($dataTable as $row)
		{
			$cartsPointsToday = (isset($pointsArray[$row['fix_date']]) ? $pointsArray[$row['fix_date']] : 0);
			$customizationToday = (isset($ordersArray[$row['fix_date']]) ? $ordersArray[$row['fix_date']]['Customization'] : 0);
			$discountToday = (isset($discountArray[$row['fix_date']]) ? $discountArray[$row['fix_date']] : 0);
			$preTaxToday = (isset($ordersArray[$row['fix_date']]) ? $ordersArray[$row['fix_date']]['totalPreTax'] : 0);
			$taxToday = (isset($ordersArray[$row['fix_date']]) ? $ordersArray[$row['fix_date']]['Tax'] : 0);
			$shippingToday = (isset($ordersArray[$row['fix_date']]) ? $ordersArray[$row['fix_date']]['Shipping'] : 0);
			$orderTotalToday = (isset($ordersArray[$row['fix_date']]) ? $ordersArray[$row['fix_date']]['orderTotal'] : 0);
			$visitsToday = (int)(isset($visitArray[$row['fix_date']]) ? $visitArray[$row['fix_date']] : 0);
			$sourcingToday = $row['totalSourcing'];
			...
			$totalHT = $orderTotalToday - $discountToday;
                        $points_to_cash = Tools::getPointsToCash(2);
			$totalCashForPoints = $cartsPointsToday * $points_to_cash * (float)$dataTable[strtotime($row['fix_date'])]['dailyRate'];
			$totalHT = $orderTotalToday + ($totalCashForPoints) + $customizationToday;
			...
			<tr>
				...
				<td align="right" >'.Tools::displayPrice($sourcingToday, $currency).'</td>
				<td align="right" >'.Tools::displayPrice($preTaxToday, $currency).'</td>
				<td align="right" >'.Tools::displayPrice($taxToday, $currency).'</td>
				<td align="right" >'.Tools::displayPrice($shippingToday, $currency).'</td>
				<td align="right" >'.Tools::displayPrice($customizationToday, $currency).'</td>
				<td align="right" >'.Tools::displayPrice($totalCashForPoints, $currency).'</td>
			...
			$this->t9 += $preTaxToday;
			$this->t10 += $taxToday;
			$this->t11 += $shippingToday;
			$this->t12 += $cartsPointsToday;
			$this->t13 += $customizationToday;
			$this->t14 += $sourcingToday;
			$this->t15 += $totalCashForPoints;
		}
		...
		$this->_html .= '
				<tr>
					...
					<th style="width:100px;text-align:center">'.$this->l('Sourcing Total').'</th>
					<th style="width:100px;text-align:center">'.$this->l('Pre-tax Total').'</th>
					<th style="width:100px;text-align:center">'.$this->l('Tax').'</th>
					<th style="width:100px;text-align:center">'.$this->l('Shipping').'</th>
					<th style="width:100px;text-align:center">'.$this->l('Cust.').'</th>
					<th style="width:100px;text-align:center">'.$this->l('Coins.').'</th>
					<th style="width:100px;text-align:center">'.$this->l('Total Sales').'</th>
				</tr>
				<tr>
					<th>'.$this->l('Total').'</th>
					...
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t14, $currency).'</td>
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t9, $currency).'</td>
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t10, $currency).'</td>
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t11, $currency).'</td>
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t13, $currency).'</td>
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t15, $currency).'</td>
				...
				<tr>
					...
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t14 / $intervalAvg, $currency).'</td>
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t9 / $intervalAvg, $currency).'</td>
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t10 / $intervalAvg, $currency).'</td>
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t11 / $intervalAvg, $currency).'</td>
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t13 / $intervalAvg, $currency).'</td>
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t15 / $intervalAvg, $currency).'</td>
				...
				<tr>
					...
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t14 * $prop30, $currency).'</td>
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t9 * $prop30, $currency).'</td>
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t10 * $prop30, $currency).'</td>
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t11 * $prop30, $currency).'</td>
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t13 * $prop30, $currency).'</td>
					<td style="font-weight: 700" align="right">'.Tools::displayPrice($this->t15 * $prop30, $currency).'</td>
			...
			foreach ($ca['payment'] as $payment)
				$this->_html .= '
					<tr>
						...
						<td style="text-align:center;padding:4px;width:100px">'.(int)($payment['nb']).'<br />'.number_format((100 * $payment['nb'] / $ca['ventil']['nb']), 1, '.', ' ').' %</td>
						<td style="text-align:center;padding:4px;width:100px">'.Tools::displayPrice($payment['total'], $currency).'<br />'.number_format((100 * $payment['total'] / $ca['ventil']['total']), 1, '.', ' ').' %</td>
						<td style="text-align:center;padding:4px;width:100px">'.Tools::displayPrice($payment['cart'], $currency).'</td>
					</tr>';
			$this->_html .= '
			</table>
		</fieldset>
	...
	private function getRealCA()
	{
		...
		SELECT SUM(od.`product_price` * od.`product_quantity` * IFNULL(pr.conversion_rate,55) / o.conversion_rate) as orderSum, COUNT(*) AS orderQty, cl.name, AVG(od.`product_price` * IFNULL(pr.conversion_rate,55)/ o.conversion_rate) as priveAvg
		FROM `'._DB_PREFIX_.'orders` o
		LEFT JOIN `'._DB_PREFIX_.'order_detail` od ON o.id_order = od.id_order
		...
		foreach ($languages as $language)
			$langValues .= 'SUM(IF(o.id_lang = '.(int)$language['id_lang'].', total_products * IFNULL(pr.conversion_rate,55) / o.conversion_rate, 0)) as '.pSQL($language['iso_code']).',';
		...
		SELECT module, SUM(total_products * IFNULL(pr.conversion_rate,55) / o.conversion_rate) as total, COUNT(*) as nb, AVG(total_products * IFNULL(pr.conversion_rate,55)/ o.conversion_rate) as cart
		FROM `'._DB_PREFIX_.'orders` o
		'.((int)$cookie->stats_id_zone ? $join : '').'
        LEFT JOIN '._DB_PREFIX_.'currency_rates pr ON date(pr.day) = date(o.invoice_date)
		WHERE o.valid = 1
		...
		SELECT z.name, SUM(o.total_products * IFNULL(pr.conversion_rate,55)/ o.conversion_rate) as total, COUNT(*) as nb
		FROM `'._DB_PREFIX_.'orders` o
		...
		SELECT cu.name, SUM(o.total_products * IFNULL(pr.conversion_rate,55)/ o.conversion_rate) as total, COUNT(*) as nb
		FROM `'._DB_PREFIX_.'orders` o
		...
		SELECT SUM(total_products * IFNULL(pr.conversion_rate,55) / o.conversion_rate) as total, COUNT(*) AS nb
		FROM `'._DB_PREFIX_.'orders` o
        LEFT JOIN '._DB_PREFIX_.'currency_rates pr ON date(pr.day) = date(o.invoice_date)
		WHERE o.valid = 1
		AND o.`invoice_date` BETWEEN '.ModuleGraph::getDateBetween());
