...
class CategoriesBar extends Module
{
	...
	*/
	function hookTop($params)
	{
		...
		$memcache_key = 'fragment-top-categories-bar';
		if (isset($_GET['id_category']))
			$memcache_key .= intval($_GET['id_category']);
		if($memcache->connect('localhost', 11211))
		{
			$html_fragment = $memcache->get('test');
			if($html_fragment) return $html_fragment;
			else 
			{
				$html_fragment = $this->getRenderedFragment($params);
				$memcache->set($memcache_key, $html_fragment, false, 3600);
