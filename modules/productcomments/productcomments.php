			...
			if (!$customerComment OR ($customerComment AND (strtotime($customerComment['date_add']) + Configuration::get('PRODUCT_COMMENTS_MINIMAL_TIME')) < time()))
			{
				...
				if (!sizeof($errors) AND Tools::getValue('content'))
				{
					...
					else
					{
						...
						Mail::Send((int)($cookie->id_lang), 'review', Mail::l('New Product Review'),
							array('{product}' => $product->name, '{rw_rating}' => $comment->grade, '{title}' => $comment->title,
							'{rw_content}' => $comment->content), 'reviews@violetbag.com', 'Product Reviews');
					}
				}
				else
