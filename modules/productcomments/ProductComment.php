...
class ProductComment extends ObjectModel
{
	 ...
	 * @return array Comments
	 */
	static public function getComments($pageNumber=1, $pageSize=20)
	{
		global $cookie;
		
		return (Db::getInstance()->ExecuteS('
		SELECT pc.`id_product_comment`, pc.`id_product`, IF(c.id_customer, CONCAT(c.`firstname`, \' \',  c.`lastname`), pc.customer_name) customer_name, 
		pc.`content`, pc.`grade`, pc.`date_add`, pc.`title`, pl.`name`, pc.validate, pc.deleted, c.id_customer as customerID
		FROM `'._DB_PREFIX_.'product_comment` pc
		LEFT JOIN `'._DB_PREFIX_.'customer` c ON (c.`id_customer` = pc.`id_customer`) 
		LEFT JOIN `'._DB_PREFIX_.'product_lang` pl ON (pl.`id_product` = pc.`id_product`) 
	...
	{
		$result = $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
		SELECT count(*) as "total" from`'._DB_PREFIX_.'product_comment` ');
