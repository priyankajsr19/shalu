...
{	
	...
	function hookOrderConfirmation($params)
	{
		...
		$trans = array('id' => intval($order->id),				// order ID - required
						...
						'total' => Tools::ps_round(floatval($order->total_paid) / floatval($conversion_rate), 0),		// total - required
						'tax' => '0', // tax
						'shipping' => Tools::ps_round(floatval($order->total_shipping) / floatval($conversion_rate), 0),	// shipping
			...
			foreach ($products AS $product)
			{
				...
				$items[] = array('OrderId' => intval($order->id),				// order ID - required
								...
								'Price' => Tools::ps_round(floatval($product['product_price_wt']) / floatval($conversion_rate), 0),	// unit price - required
