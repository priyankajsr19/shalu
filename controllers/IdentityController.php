...
class IdentityControllerCore extends FrontController
{
	...
	public function preProcess()
	{
		...
		if (sizeof($_POST))
		{
			$exclusion = array('secure_key', 'old_passwd', 'passwd', 'active', 'date_add', 'date_upd', 'last_passwd_gen', 'newsletter_date_add', 'id_default_group', 'total_delivered');
			$fields = $customer->getFields();
			foreach ($fields AS $key => $value)
				if (!in_array($key, $exclusion))
					$customer->{$key} = key_exists($key, $_POST) ? trim($_POST[$key]) : 0;
            if( isset($_POST['newsletter']) && (int)$_POST['newsletter'] === 0 )
	...
	public function displayContent()
	{
		...
		if($isBetaUser)
			self::$smarty->display(_PS_THEME_DIR_.'beta/identity.tpl');
		else 
