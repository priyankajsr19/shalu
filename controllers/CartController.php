...
class CartControllerCore extends FrontController
{
	...
	public function preProcess()
	{
		...
		// remove free gift if found in cart
		if(self::$cart->containsProduct(FREE_GIFT_ID, NULL, NULL))
		    self::$cart->deleteProduct(FREE_GIFT_ID);
				...
				else
				{
					...
					if ($idProductAttribute AND is_numeric($idProductAttribute))
					{
						...
						if( !$delete ) {
							...
							if( $cv < 50 && in_array($producToAdd->id,$free_products)) {	
								$this->errors[] = Tools::displayError("Free Gift Product is allowed only if the shopping cart value is more than USD 50");
							} else {
								...
								foreach($cart_products as $cproduct) {
									...
									if( in_array($id_product, $free_products )) {
										$this->errors[] = Tools::displayError("Only one Free Gift allowed per order");
										break;
									}
								}
							}
