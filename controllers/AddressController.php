...
class AddressControllerCore extends FrontController
{
	...
	public function preProcess()
	{
		...
		if (Tools::isSubmit('submitAddress'))
		{
			...
			*/
			...
			if($op == 0 && $cod == 0) $this->errors[] = 'We are sorry but we do not provide service to this region as of now. However, we keep adding new locations with time and would request you to check our website a few weeks later. We apologise for the inconvenience caused. ';
	...
	public function process()
	{
		...
		elseif (isset($this->_address) AND isset($this->_address->id_country) AND !empty($this->_address->id_country) AND isset($this->_address->id_state))
			$selected_state = $this->_address->id_state;
		else 
			$selected_state = 244; // default to karnataka.
		...
		self::$smarty->assign('address_count', sizeof($customer->getAddresses((int)(self::$cookie->id_lang))));
	}
